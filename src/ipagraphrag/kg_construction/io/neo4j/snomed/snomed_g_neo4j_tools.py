...
import base64, optparse, datetime
import sys
import pandas as pd
...
def run_cypher(arglist):
  ...
  n4jpw = opts.neopw
  try:
    graph_db = GraphDatabase.driver("bolt://localhost:7687", auth=("neo4j", n4jpw)).session()
  ...
  for idx,cmd in enumerate(command_lines):
    ...
    if cmd.startswith('<FILE>'):
      file_path = cmd.replace('<FILE>','')
      print("File path: {}".format(file_path))
      current_data = pd.read_csv(file_path, keep_default_na=False)
    ...
    try:
      if current_data is None:
        graph_db.run(next_cmd) # returns Cursor, empirical evidence (not in py2neo doc)
      else:
        tuples = 0
        for index, r in current_data.iterrows():
          if tuples % 1000 == 0:
            tx = graph_db.begin_transaction()
          values = r.to_dict()
          tx.run(next_cmd, values)
          tuples += 1
          if tuples % 1000 == 0 or tuples == len(current_data):
            tx.commit()
            tx.close()
        current_data = None
