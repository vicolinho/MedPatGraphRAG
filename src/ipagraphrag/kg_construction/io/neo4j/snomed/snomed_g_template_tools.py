...
def instantiate(arglist):
  ...
  opt.add_option('--neo4j_import_path', action='store', dest='neo4j_import_path', default="./")
  ...
  config['output_dir'] = opts.neo4j_import_path
  ...
  with open(output_dir+'used_roles.csv') as f:
    for idx,line in enumerate(x.rstrip('\n').rstrip('\r') for x in f):
      ...
      load_csv_line = ("<FILE>%s%sDR_%s_new.csv"% (('file:///' if sys.platform in ['cygwin','win32','darwin'] else 'file:'),output_dir,typeId)).replace('file:////','file:///')
      print(load_csv_line, file=fout)
      #print('CALL {',file=fout)
      #print('  with line ',file=fout)
      print('  MERGE (rg:RoleGroup { sctid: $sctid, rolegroup: $rolegroup });',file=fout)
      ...
      #print('CALL {',file=fout)
      #print('  with line ',file=fout)
      print('  MATCH (rg:RoleGroup { sctid: $sctid, rolegroup: $rolegroup })',file=fout)
      print(  'WITH rg ',file=fout)
      print('  MATCH (c:ObjectConcept { sctid: $destinationId })',file=fout)
      print('  MERGE (rg)-[:%s { id: $id, active: $active, sctid: $sctid,' % rolename,file=fout)
      print('                               typeId: $typeId,',file=fout)
      print('                               rolegroup: $rolegroup, effectiveTime: $effectiveTime,',file=fout)
      print('                               moduleId: $moduleId, characteristicTypeId: $characteristicTypeId,',file=fout)
      print('                               modifierId: $modifierId,',file=fout)
      print('                               history: $history }]->(c);',file=fout)
      #print(' } IN TRANSACTIONS OF 200 ROWS;',file=fout)
  # close CSV, wrap up
