...
def db_build(arglist):
  ...
  opt.add_option('--language_code', action='store', default='en',
                 choices=['en','de', 'en-us', 'en-US', 'en-GB'])
  ...
  commands_d = {
    ...
    'TEMPLATE_PROCESSING':
        {'stepname': 'TEMPLATE_PROCESSING',
         'cmd':      'python %s/snomed_g_template_tools.py instantiate %s/snomed_g_graphdb_cypher_%s_2.template build.cypher --rf2 %s --release_type %s' \
                         % (snomed_g_bin, snomed_g_bin, ('create' if opts.action=='create' else 'update'), opts.rf2, opts.release_type),
         'mode':     ['build','prep']},
    'CYPHER_EXECUTION':
        {'stepname': 'CYPHER_EXECUTION',
         'cmd':      '%s %s/snomed_g_neo4j_tools.py run_cypher build.cypher --verbose --neopw %s' \
                         % (sys.executable, snomed_g_bin, opts.neopw),
