...
class Neo4j_Access:
  def __init__(self, base64pw):
    # NEO4J init
    self.graph_db = GraphDatabase.driver("bolt://localhost:7687", auth=("neo4j", base64pw)).session()
    result = [r for r in self.graph_db.run("""Call dbms.listConfig() YIELD name, value WHERE name='server.directories.neo4j_home' RETURN value""")]

    self.neo4j_home = os.path.join(result[0]['value'], 'import')
    #self.graph_db = (
    #  py2neo.Graph("bolt://localhost:7687", auth=("neo4j", base64pw)))  # 'http://localhost:7474/db/data/transaction/commit'
  ...
  def lookup_elements_by_id(self,query_template,query_target_variable,id_field_name,id_list,chunk_size):
    ...
    for chunk in [id_list[idx:idx+n] for idx in range (0, len(id_list), n)]:
      ...
      for record in cursor:
        idx += 1
        r = record[query_target_variable] # variable in CYPHER query
